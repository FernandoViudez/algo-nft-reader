...
export class Asset {
  ...
  async getDigitalMedia(): Promise<ASADigitalMedia[]> {
    switch (this.arc) {
      ...
      case ArcEnum.arc69: {
        return await Arc69.getDigitalMedia(this.info, this.indexerService);
      }
      default: {
        return [
          {
            media: this.info.params.url,
            integrity: this.info.params.url.startsWith('ipfs://')
              ? undefined
              : this.info.params['metadata-hash'],
          },
        ];
      }
    }
